...
package Manoc::Form::VlanRange::Split;
...
sub validate {
	...
	$self->field('split_point')->add_error("Split point should be withing the range $item_start-$item_end");
    }
}
...
override 'update_model' => sub {
    ...
    return $self->item->split_new_range(
        $values->{new_name},
        $values->{split_point},
        $values->{direction},
    );
