...
BEGIN { use_ok 'App::Manoc::DB::Search::Query' }

my ( $q, $s );

ok( $q = App::Manoc::DB::Search::Query->new( { search_string => 'test' } ), 'Object creation' );

$q = App::Manoc::DB::Search::Query->new( { search_string => 'word' } );
$q->parse;
ok( @{ $q->words() } == 1, 'Tokenizer one word' );

$s = 'more than two words';
$q = App::Manoc::DB::Search::Query->new( { search_string => $s } );
$q->parse;
